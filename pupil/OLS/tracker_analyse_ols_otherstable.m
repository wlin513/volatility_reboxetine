...
%     saveas(f1,[figdir,'loss_',num2str(sub),'_designmatrix_othervol.png'])
 ...
 %% runing the ols for each block separately (not excluding first 5 trials for each block)
    ...
    bad_trials_rew=bad_trials_rew|lossvol==2;
    bad_trials_loss=bad_trials_loss|winvol==2;
...
save('olsresult_otherstable.mat');
%load('olsresult.mat');
%% figures
...
H=findobj(gca,'Type','text');
set(H,'Rotation',60); % tilt
saveas(f3,[figdir,'/mean_designmatrix_otherstable.png'])
%%
...
% saveas(f1,[figdir,group{1},'visit',num2str(visit),'ols_out_all_EVs_otherstable.png'])
% 
% 
%% plot each EV
for group={'reboxetine','placebo'}
        for visit=1:2
            ...
            saveas(f2,[figdir,'/',strrep(EVs{i},'*','_'),'_',group{1},'_visit_',num2str(visit),'ols_out_EV_','_otherstable.png'])
            end
        end
end
%%
...
for group={'reboxetine','placebo'}
    ...
    saveas(f2,[figdir,'/',strrep(EVs{i},'*','_'),'_',group{1},'_diff_ols_out_EV_','_otherstable.png'])
    end
end
% 
% 
...
% 
% saveas(f4,[figdir,'ols_out_EV_block_',strrep(EVs_block{i},'*','_'),'_otherstable.png'])
% end
% 
...
% %saveas(fall,[figdir,'pupil responses all trials original_otherstable.png'])
% subplot(4,1,3)
...
% 
% 
% saveas(fbetablock1,[figdir,'ols-',EVs{i},' each block separately_otherstable.png'])
% end
% 
% 
% %%
...
% plot(range_real*1000,[0 0],'k--') 
% legend('win-received','loss-received')
% saveas(f_all_real,[figdir,'pupil responses all trials real_otherstable.png'])
% %%
% f_rec_real=figure;
% 
...
% 
% 
% saveas(f_rec_real,[figdir,'pupil response each block separately real_otherstable.png'])
% %%
% f11=figure;
% 
...
% 
% 
% saveas(f11,[figdir,'pupil response each block separately_otherstable.png'])
% %%
...
% 
% saveas(fswitchblockreal,[figdir,'pupil response to switch each block separately real_otherstable.png'])
% 
% %%
...
% 
% 
% saveas(fswitchblock1,[figdir,'pupil response to switch each block separately_otherstable.png'])
% 
% % nf=figure;
% % hold on;
