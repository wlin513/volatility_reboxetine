...
saveas(f,[figdir,'loss_selfv_otherv_PE_reboxetine_othervol.png'])
...
plot(timelab,zeros(size(timelab)),'k--');
legend('visit 1','visit 2')
saveas(f,[figdir,'loss_selfv_otherv_PE_placebo_othervol.png'])
...
hold on
plot_ev(lossbeta.placebo(2,:,7,501:end)-lossbeta.placebo(1,:,7,501:end),timelab,'visit2 vs. visit1: other-volatile','Coeffients for self-vol*loss-PE',[0.937,0.863, 0.020],true,2)
hold on
plot(timelab,zeros(size(timelab)),'k--');
legend('reboxetine','placebo')
saveas(f,[figdir,'loss_selfv_otherv_PE_reboxetine_v2minusv1_vs_placebo_othervol.png'])