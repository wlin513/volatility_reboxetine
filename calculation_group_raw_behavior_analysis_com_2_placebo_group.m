...
%sublist.reboxetine=sublist.all(Ques.treatment==1);
 sublist.placebo1=Ques.subnum(Ques.treatment==2);
% %exclude 
% excludesub=[2014,1011];%1021
% for group={'reboxetine','placebo1','all'}
...
% end
%%
load([datadir,'rivastigmine_control_data/subjectsGood.mat'])
sublist.placebo2=subjectsGood(subjectsGood(:,2)==4,1);
sublist.all=[sublist.placebo1;sublist.placebo2];
%%
addpath("plot_code/")
...
for group={'placebo1','placebo2','all'}
            ...
            for ss=1:size(ssublist,1)
                   if ssublist(ss)<8000
                    ...
                    data=read_txt([sdatadir,filename.name]);
                     ...
                     filename=dir([datadir,'rivastigmine_control_data/',num2str(ssublist(ss)),'_visit_',num2str(v),'_blktype_*.txt']);

                    data=read_txt([datadir,'rivastigmine_control_data/',filename.name]);
                end
...
T_s=table(sublis.all,[ones(size(sublist.placebo1));2*ones(size(sublist.placebo2))],'VariableNames',{'sub','treatment'});
T=[T_s,T_pct_wvsl,T_pct_wvsl_diff,T_nowin,T_loss,T_win,T_noloss,T_winvsnowin,T_nolossvsloss];
writetable(T,[datadir,'switchprob_2placebos'])
%%
...
writetable(T,[datadir,'switchprob_2placebos_asin_sqrt'])
...
within=table(repelem({'win';'loss'},8,1),repmat(repelem({'visit1';'visit2'},4,1),2,1),repmat({'winv';'winv';'wins';'wins'},4,1),repmat({'lossv';'losss';'lossv';'losss'},4,1),...
 ...
 %%
  T_pct_pos=[T_s,T_win,T_noloss];
within=table(repelem({'win';'loss'},8,1),repmat(repelem({'visit1';'visit2'},4,1),2,1),repmat({'winv';'winv';'wins';'wins'},4,1),repmat({'lossv';'losss';'lossv';'losss'},4,1),...
     'VariableNames',{'valence','visit','winvol','lossvol'});
 rm=fitrm(T_pct_pos,'win_visit1_bothv-noloss_visit2_boths ~ treatment','WithinDesign',within);
 ranovatbl=ranova(rm,'WithinModel','valence*visit*winvol*lossvol')
%%
