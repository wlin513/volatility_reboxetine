    ...
    for v=1:2
            for ss=1:size(ssublist,1)
                    ...
                    for i=1:blkn
                            ...
                            oneback=[nan(1,2);information(1:end-1,:)];
                            twoback=[nan(2,2);information(1:end-2,:)];
                            threeback=[nan(3,2);information(1:end-3,:)];
                            fourback=[nan(4,2);information(1:end-4,:)];
                            fiveback=[nan(5,2);information(1:end-5,:)];
                           % sixback=[nan(6,2);information(1:end-6,:)];
                            
                            regmatrix=[oneback,twoback,threeback,fourback,fiveback];
                            opts=statset('glmfit');
                            opts.MaxIter=1000000;
                            betas.(group{1})(v,ss,i,:)=glmfit(regmatrix,choice,'binomial','link','logit');
...
saveas(f1,[figdir,'\all_visit_1_logistic_regression_on_option_chosen_comp_blocks.png'])
...
for v=1:2
    ...
    saveas(f2,[figdir,'\cmp_groups_visit_',num2str(v),'_win_logistic_regression_on_option_chosen.png'])
        %loss
            f2=figure('units','inch','position',[5,2,9,7]);
    for i=1:blkn
            subplot(2,2,i)
    ...
    saveas(f2,[figdir,'\cmp_groups_visit_',num2str(v),'_loss_logistic_regression_on_option_chosen.png'])
end
%% plot bargarph comparing groups for visit2 - visit1 each block*valence
 for group={'reboxetine','placebo'}
    ...
    %win    
    for i=1:blkn
            subplot(2,2,i)%% plot bargarph comparing groups for visit2 - visit1 each block*valence
 for group={'reboxetine','placebo'}
            betas_win.(group{1})=squeeze(betas.(group{1})(2,:,:,2:2:10)-betas.(group{1})(1,:,:,2:2:10));
            betas_loss.(group{1})=-squeeze(betas.(group{1})(2,:,:,3:2:11)-betas.(group{1})(1,:,:,3:2:11));     
            mean_beta_win.(group{1})=squeeze(mean(betas_win.(group{1})));
    ...
    saveas(f2,[figdir,'\cmp_groups_diff_win_logistic_regression_on_option_chosen.png'])
    end
        %loss
            f2=figure('units','inch','position',[5,2,9,7]);
    for i=1:blkn
    ...
    saveas(f2,[figdir,'\cmp_groups_diff_loss_logistic_regression.png'])