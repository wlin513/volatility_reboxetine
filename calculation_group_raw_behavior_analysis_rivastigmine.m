...
getfolders;
datadir='/home/wlin/Documents/2019_drug_study/data/rivastigmine_control_data/';
figdir=[figdir,'rivastigmine/'];
%% load questionnaire data 
load([datadir,'subjectsGood.mat'])
sublist.all=subjectsGood(:,1);
sublist.rivastigmine=subjectsGood(subjectsGood(:,2)==3,1);
...
for group={'rivastigmine','placebo2','all'}
    ssublist=sublist.(group{1});
    for v=1:2
            for ss=1:size(ssublist,1)

                    filename=dir([datadir,num2str(ssublist(ss)),'_visit_',num2str(v),'_blktype_*.txt']);
...
writetable(T,[datadir,'switchprob_rivastigmine'])
%%
...
writetable(T,[datadir,'switchprob_rivastigmine_asin_sqrt'])
...
within=table(repelem({'win';'loss'},8,1),repmat(repelem({'visit1';'visit2'},4,1),2,1),repmat({'winv';'winv';'wins';'wins'},4,1),repmat({'lossv';'losss';'lossv';'losss'},4,1),...
 ...
 ranovatbl=ranova(rm,'WithinModel','valence*visit*winvol*lossvol')
%%
scatter(ones(1,27),squeeze(pct_winvsloss_chosen.placebo2(2,:,4)-pct_winvsloss_chosen.placebo2(1,:,4)))
scatter(ones(1,27),squeeze(switchprob_nowin.placebo2(2,:,4)-switchprob_nowin.placebo2(1,:,4)))