...
parameters {
  ...
  real alpha_tr[2,nsub]; // learning rate - separate learning rates for wins and losses; two per participant
  real beta_tr[nsub];    // inverse temperature - ; one per participant
  real zeta_tr[2,nsub]; // learning noise scaling; two per participant
}

// We can also compute the difference between the two learning rates directly in Stan:
transformed parameters{
  real<lower=0,upper=1> alpha[2,nsub];
  real<lower=0> beta[nsub];
  real<lower=0> zeta[2,nsub];
  // real randomnum[2,ntr,nsub];
  ...
  for (is in 1:nsub){
    ...
    zeta[1,is] = inv_logit(zeta_tr[1,is]);
    zeta[2,is] = inv_logit(zeta_tr[2,is]);
  }
}
...
model {
  ...
  real winepsilon[ntr,nsub]; //trial by trial update noise
  real lossepsilon[ntr,nsub];
  // Priors for the individual subjects are the group:
  for (is in 1:nsub){
    alpha_tr[1,is] ~ normal(0,2);
    alpha_tr[2,is] ~ normal(0,2);
    beta_tr[is]  ~ normal(0,1.5);
    zeta_tr[1,is] ~ normal(0,2);
    zeta_tr[2,is] ~ normal(0,2);
    //for (it in 1:ntr){
    //randomnum[1,it,is]~ normal(0,1);
    //randomnum[2,it,is]~ normal(0,1);
    //}
  }
 ...
 // running the model is as before:
  for (is in 1:nsub){ // run the model for each subject
    ...
    for (it in 1:(ntr-1)){
      ...
      winepsilon[it,is]= fabs(winpredictionError[it,is]*zeta[1,is]);
      winpredictionOpt1[it+1,is] = winpredictionOpt1[it,is] + alpha[1,is]*(winpredictionError[it,is])+winepsilon[it,is];
      
     
      losspredictionError[it,is]  = opt1lossout[it,is]-losspredictionOpt1[it,is];
      lossepsilon[it,is]= fabs(losspredictionError[it,is]*zeta[2,is]);
      losspredictionOpt1[it+1,is] = losspredictionOpt1[it,is] + alpha[2,is]*(losspredictionError[it,is])+lossepsilon[it,is];
