...
model {
 ...
 // running the model is as before:
  for (is in 1:nsub){ // run the model for each subject
    // Learning
    winpredictionOpt1[1,is] = 0; // on the first trial, 50-50 is the best guess
    losspredictionOpt1[1,is] = 0; // on the first trial, 50-50 is the best guess
    for (it in 1:(ntr-1)){
     if (opt1winout[it,is] == opt1Chosen[it,is]){
      winpredictionError[it,is]  = -(-1)^opt1winout[it,is]-winpredictionOpt1[it,is];
     ...
     if (opt1lossout[it,is] == opt1Chosen[it,is]){
      losspredictionError[it,is]  = -(-1)^opt1lossout[it,is]-losspredictionOpt1[it,is];
