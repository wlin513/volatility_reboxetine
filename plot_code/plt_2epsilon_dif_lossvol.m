function plt_2epsilon_dif_lossvol(epsilon1,epsilon2,group,modelname,figdir)
condition={'loss volatile','loss stable'};
%calculate the mean of epsilon in logit space and storeerse it back to the
%original space
epsilon1_store1=squeeze(epsilon1.(group{1})(2,:,:)-epsilon1.(group{1})(1,:,:));
epsilon1_store=[mean(epsilon1_store1(:,1:2:3),2),mean(epsilon1_store1(:,2:2:4),2)];
...
epsilon2_store1=squeeze(epsilon2.(group{1})(2,:,:)-epsilon2.(group{1})(1,:,:));
epsilon2_store=[mean(epsilon2_store1(:,1:2:3),2),mean(epsilon2_store1(:,2:2:4),2)];
...
H(1).FaceColor = 'green';
H(2).FaceColor = 'red';
set(gca,'XTickLabel',condition,'FontSize',13);
...
saveas(f,[figdir,group{1},'_epsilon_dif_lossvol_result_',modelname,'.png'])
